...
def _parse_form(form) -> tuple[dict, dict, dict]:
    ...
    project_data = dict(
        ...
        phone=s("phone"),
        email=s("email"),
        invoice_type=InvoiceType(form.get("invoice_type", InvoiceType.maganszemely.value)),
        status=ProjectStatus(form.get("status", ProjectStatus.felmerendo.value)),
        priority=Priority(form.get("priority", Priority.normal.value)),
        job_type=JobType(form.get("job_type", JobType.terasz.value)),
        distributor=Distributor(form.get("distributor", Distributor.royal.value)),
        crew=Crew(form.get("crew", Crew.nincs.value)),
